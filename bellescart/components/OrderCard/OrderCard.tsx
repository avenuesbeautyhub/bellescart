      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        ...
        <div className="flex items-start gap-3">
          <div className="w-10 h-10 bg-gray-100 rounded-lg flex items-center justify-center flex-shrink-0 overflow-hidden">
            {order.items?.[0]?.product?.images?.[0]?.url || order.items?.[0]?.product?.image ? (
              <img
                src={order.items?.[0]?.product?.images?.[0]?.url || order.items?.[0]?.product?.image}
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pt-4 border-t border-gray-100">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <div className={`mt-1 flex items-center gap-2 text-sm font-semibold ${(order.paymentStatus === 'paid' || order.paymentStatus === 'completed') ? 'text-green-600' : 'text-amber-600'}`}>
