...
export default function DashboardPage() {
  ...
  // Load current user data and check localStorage states
  useEffect(() => {
    if (!mounted) return;
  ...
  const isReady = mounted && loaded && isAuthenticated;
            ...
            <section className="container mx-auto px-4 py-12">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                {recentProducts.map((product, index) => (
                  <Link key={product._id || `product-${index}`} href={`/product?id=${product._id}`}>
